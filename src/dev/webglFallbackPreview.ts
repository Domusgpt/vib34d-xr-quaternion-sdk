...
import PreviewTelemetry from './preview/PreviewTelemetry.ts';
...
export default class WebGLFallbackPreview {
  ...
  private readonly telemetry: PreviewTelemetry;
  ...
  constructor(options: WebGLFallbackPreviewOptions) {
    ...
    this.telemetry = new PreviewTelemetry();
    this.telemetry.setQuaternion(this.state.quaternion);
    this.telemetry.setAudioBands(this.state.audio);
    this.telemetry.setVisualParams(this.state.visual);
    this.telemetry.setConfidence(this.state.confidence);
  }
  ...
  getPredictionSnapshot(): { confidence: number; horizonMs: number; rotor: readonly [number, number, number] } | null {
    return this.telemetry.getPredictionSnapshot();
  }
  ...
  private readonly handleFrame = (timestamp: number) => {
    ...
    this.render(timestamp * 0.001, deltaTime);
    this.updateFps(deltaTime);

    this.rafHandle = requestAnimationFrame(this.handleFrame);
  };

  private render(timeSeconds: number, deltaTime: number): void {
    this.resize();

    this.gl.useProgram(this.program);
    this.gl.clear(this.gl.COLOR_BUFFER_BIT);

    const telemetry = this.telemetry.update(timeSeconds, deltaTime);
    this.state = {
      ...this.state,
      audio: { ...telemetry.audio },
      visual: { ...telemetry.visual },
    ...
    const rotationSpeed = telemetry.visual.rotationSpeed * (0.7 + telemetry.audio.high * 0.8) + Math.abs(roll) * 0.15;
    const morphFactor = clamp01(telemetry.visual.morphFactor + attitudeEnergy * 0.18 + telemetry.audio.mid * 0.25);
    const dimension = telemetry.visual.dimension + telemetry.audio.energy * 0.9 + telemetry.pulse * 0.5;
    const universeModifier = telemetry.visual.universeModifier + telemetry.pulse * 0.35;
    const patternIntensity = 0.8 + telemetry.visual.universeModifier * 0.2 + telemetry.pulse * 0.6;
    const gridDensity = 6.5 + universeModifier * 2.4 + telemetry.audio.bass * 4.2;
    const colorShift = (telemetry.audio.high - telemetry.audio.bass) * 0.35;
    const glitchIntensity = clamp01((1 - telemetry.confidence) * 0.35 + telemetry.drift * 0.4);
