...
export default class WebGLFallbackPreview {
  ...
  private materialOverrides: MaterialOverrides = {};
  ...
  setMaterialOverrides(overrides: MaterialOverrides): void {
  ...
  listRisks(): string[] {
    return [
      'WebGPU unavailable — rendering with WebGL fallback shader.',
  ...
  private render(timeSeconds: number): void {
    ...
    const patternIntensity = Number.isFinite(this.materialOverrides.patternIntensity)
      ? this.materialOverrides.patternIntensity ?? 0
      : 0.8 + visual.universeModifier * 0.2 + this.pulseIntensity * 0.6;
    const gridDensity = Number.isFinite(this.materialOverrides.gridDensity)
      ? this.materialOverrides.gridDensity ?? 0
      : 6.5 + universeModifier * 2.4 + audio.bass * 4.2;
    const lineThickness = Number.isFinite(this.materialOverrides.lineThickness)
      ? this.materialOverrides.lineThickness ?? 0
      : lerp(0.02, 0.045, audio.energy);
    const shellWidth = Number.isFinite(this.materialOverrides.shellWidth)
      ? this.materialOverrides.shellWidth ?? 0
      : lerp(0.02, 0.06, audio.mid);
    const tetraThickness = Number.isFinite(this.materialOverrides.tetraThickness)
      ? this.materialOverrides.tetraThickness ?? 0
      : lerp(0.02, 0.05, audio.energy);
    const colorShift = Number.isFinite(this.materialOverrides.colorShift)
      ? this.materialOverrides.colorShift ?? 0
      : (audio.high - audio.bass) * 0.35;
    const glitchIntensity = Number.isFinite(this.materialOverrides.glitchIntensity)
      ? Math.max(0, this.materialOverrides.glitchIntensity ?? 0)
      : (1 - confidence) * 0.45;
    ...
    this.setUniform1f('u_lineThickness', lineThickness);
    this.setUniform1f('u_shellWidth', shellWidth);
    this.setUniform1f('u_tetraThickness', tetraThickness);
    ...
    const primary = this.materialOverrides.primaryColor ?? [0.95, 0.25, 0.85];
    const secondary = this.materialOverrides.secondaryColor ?? [0.25, 0.95, 0.95];
    const background = this.materialOverrides.backgroundColor ?? [0.05, 0.02, 0.18];

    this.setUniform3f('u_primaryColor', primary[0], primary[1], primary[2]);
    this.setUniform3f('u_secondaryColor', secondary[0], secondary[1], secondary[2]);
    this.setUniform3f('u_backgroundColor', background[0], background[1], background[2]);
