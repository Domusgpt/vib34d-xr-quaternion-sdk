...
export function createQuaternionPreview(
  ...
  const initializeWebGPU = async () => {
    ...
    if (!navGpu?.requestAdapter) {
      webgpuStatus.textContent = 'WebGPU not available in this browser.';
      updateRiskList([]);
      return;
    }

    try {
      webgpuHarness = await WebGPUPreviewHarness.create({
        canvas: webgpuCanvas,
        onFrame: ({ fps }) => {
  ...
  const emitUpdate = () => {
    ...
    updatePrediction(webgpuHarness?.getPredictionSnapshot() ?? null);
