...
class MockQuaternionSystem {
  ...
  setActive(active: boolean) {
    this.active = !!active;
    if (this.active) {
      this.root.classList.add('qp-system--active');
    } else {
      this.root.classList.remove('qp-system--active');
    }
  }
...
const dispatchSystemEvent = (eventName: string, detail: Record<string, unknown>) => {
  ...
  try {
    ...
    if (typeof console !== 'undefined' && typeof console.warn === 'function') {
      console.warn('Failed to dispatch system event', eventName, error);
    }
    return;
  }
...
export function createQuaternionPreview(
  ...
  const systemSelectOptions = [
    { value: 'quantum', label: 'Quantum engine' },
    { value: 'holographic', label: 'Holographic engine' },
    { value: 'faceted', label: 'Faceted system' },
  ] as const;
  type SystemId = (typeof systemSelectOptions)[number]['value'];

  const systemSelect = createSelect<SystemId>(
    controls,
    'Active visualization system',
    systemSelectOptions,
    systemSelectOptions[0].value,
  );

  const initialSystem = systemSelect.getValue();
  ...
  } satisfies Record<SystemId, MockQuaternionSystem>;
  ...
  const synchronizer = new ShaderQuaternionSynchronizer({
    ...
    targetSystems: [initialSystem],
    activationEventTarget: typeof window !== 'undefined' ? window : undefined
  });
  ...
  const setActiveSystem = (
    ...
    if (!next || !(next in systems)) {
      return;
    }

    const { emitEvents = true, force = false } = options;
    const previous = activeSystem;

    if (systemSelect.select.value !== next) {
      systemSelect.select.value = next;
    }
    ...
    if (previous && systems[previous]) {
      systems[previous].setActive(false);
      if (emitEvents) {
        dispatchSystemEvent('vib34d:system-deactivated', {
          systemName: previous,
          systems: [previous]
        });
      }
    ...
    activeSystem = next;
    systems[next].setActive(true);
    synchronizer.activateExclusiveSystem(next);

    if (emitEvents) {
      dispatchSystemEvent('vib34d:system-activated', {
        systemName: next,
        systems: [next],
        engineReady: true
      });
  ...
  const handleSystemSelectChange = () => {
    const next = systemSelect.getValue();
    setActiveSystem(next);
  };

  systemSelect.select.addEventListener('change', handleSystemSelectChange);

  setActiveSystem(initialSystem, { force: true });
  ...
  return {
    ...
    synchronizer,
    destroy() {
      systemSelect.select.removeEventListener('change', handleSystemSelectChange);
      if (activeSystem && systems[activeSystem]) {
        dispatchSystemEvent('vib34d:system-deactivated', {
          systemName: activeSystem,
          systems: [activeSystem]
        });
        systems[activeSystem].setActive(false);
        activeSystem = null;
      }
      synchronizer.stop();
