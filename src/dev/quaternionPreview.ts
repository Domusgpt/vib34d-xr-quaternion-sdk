...
type SystemId = 'quantum' | 'holographic' | 'faceted';
...
class MockQuaternionSystem {
  ...
  setActive(active: boolean) {
    this.root.classList.toggle('qp-system--inactive', !active);
    this.root.setAttribute('data-active', active ? 'true' : 'false');
  }
...
const GEOMETRY_MODULES: readonly GlassGeometryModule[] = [
  'hypercube',
  'hypersphere',
  'hypertetrahedron',
];

const PROJECTION_MODULES: readonly GlassProjectionModule[] = [
  'perspective',
  'orthographic',
  'stereographic',
];

const isGeometryModule = (value: string): value is GlassGeometryModule =>
  (GEOMETRY_MODULES as readonly string[]).includes(value);

const isProjectionModule = (value: string): value is GlassProjectionModule =>
  (PROJECTION_MODULES as readonly string[]).includes(value);
...
const sanitizeMaterialControl = (
  candidate: unknown,
  fallback: LayerMaterialControl
): LayerMaterialControl => {
  if (!candidate || typeof candidate !== 'object') {
    return cloneMaterialControl(fallback);
  }
  const next = cloneMaterialControl(fallback);
  const source = candidate as Partial<LayerMaterialControl>;
  if (typeof source.primaryColor === 'string') {
    next.primaryColor = source.primaryColor;
  }
  if (typeof source.secondaryColor === 'string') {
    next.secondaryColor = source.secondaryColor;
  }
  if (typeof source.backgroundColor === 'string') {
    next.backgroundColor = source.backgroundColor;
  }
  if (typeof source.patternIntensity === 'number' && Number.isFinite(source.patternIntensity)) {
    next.patternIntensity = source.patternIntensity;
  }
  if (typeof source.glitchIntensity === 'number' && Number.isFinite(source.glitchIntensity)) {
    next.glitchIntensity = source.glitchIntensity;
  }
  if (typeof source.colorShift === 'number' && Number.isFinite(source.colorShift)) {
    next.colorShift = source.colorShift;
  }
  if (typeof source.gridDensity === 'number' && Number.isFinite(source.gridDensity)) {
    next.gridDensity = source.gridDensity;
  }
  if (typeof source.lineThickness === 'number' && Number.isFinite(source.lineThickness)) {
    next.lineThickness = source.lineThickness;
  }
  if (typeof source.shellWidth === 'number' && Number.isFinite(source.shellWidth)) {
    next.shellWidth = source.shellWidth;
  }
  if (typeof source.tetraThickness === 'number' && Number.isFinite(source.tetraThickness)) {
    next.tetraThickness = source.tetraThickness;
  }
  return next;
};
...
export function createQuaternionPreview(
  ...
  const layerStates: LayerState[] = LAYER_BLUEPRINTS.map((blueprint, index) => {
    ...
    const fallbackMaterial = presetLayer
    ...
    const persistedLayer = persistedState?.layers?.[index];
    if (persistedLayer) {
      if (typeof persistedLayer.geometry === 'string' && isGeometryModule(persistedLayer.geometry)) {
        state.geometry = persistedLayer.geometry;
      }
      if (
        typeof persistedLayer.projection === 'string' &&
        isProjectionModule(persistedLayer.projection)
      ) {
        state.projection = persistedLayer.projection;
      }
      state.material = sanitizeMaterialControl(persistedLayer.material, fallbackMaterial);
    }
  ...
  if (persistedState?.activePresetId) {
    const candidate = persistedState.activePresetId;
    const matchesPreset = PREVIEW_PRESETS.some(preset => preset.id === candidate);
  ...
  if (activePresetId !== (defaultPreset?.id ?? CUSTOM_PRESET_ID)) {
    presetSelect.select.value = activePresetId;
  }
  if (activePresetId === CUSTOM_PRESET_ID) {
    presetDescription.textContent = customPresetDescription;
  } else {
    const preset = PREVIEW_PRESETS.find(candidate => candidate.id === activePresetId);
    if (preset) {
  ...
  if (persistedState) {
    if (typeof persistedState.selectedGeometry === 'string' && isGeometryModule(persistedState.selectedGeometry)) {
      selectedGeometry = persistedState.selectedGeometry;
      if (layerStates[0]) {
        layerStates[0].geometry = persistedState.selectedGeometry;
      }
    }
    if (
      typeof persistedState.selectedProjection === 'string' &&
      isProjectionModule(persistedState.selectedProjection)
    ) {
      selectedProjection = persistedState.selectedProjection;
      if (layerStates[0]) {
        layerStates[0].projection = persistedState.selectedProjection;
      }
    }
  }
  ...
  const systemToggleCard = document.createElement('section');
  systemToggleCard.className = 'qp-system-toggle';
  const systemToggleHeading = document.createElement('h3');
  systemToggleHeading.textContent = 'Active visualization system';
  systemToggleCard.appendChild(systemToggleHeading);

  const systemToggleOptions = document.createElement('div');
  systemToggleOptions.className = 'qp-system-toggle-options';
  ...
  SYSTEM_BLUEPRINTS.forEach(({ id, label }) => {
    ...
    option.className = 'qp-system-toggle-option';

    const input = document.createElement('input');
    input.type = 'radio';
    input.name = 'qp-system-toggle';
    input.value = id;
    input.checked = id === activeSystemId;
    ...
    systemToggleOptions.appendChild(option);

    systemToggleInputs.set(id, input);
  });

  systemsColumn.appendChild(systemToggleCard);
  ...
  const persistedAngles = persistedState
    ? {
        yaw:
          typeof persistedState.yaw === 'number' && Number.isFinite(persistedState.yaw)
            ? clampRange(persistedState.yaw, -180, 180)
            : undefined,
        pitch:
          typeof persistedState.pitch === 'number' && Number.isFinite(persistedState.pitch)
            ? clampRange(persistedState.pitch, -90, 90)
            : undefined,
        roll:
          typeof persistedState.roll === 'number' && Number.isFinite(persistedState.roll)
            ? clampRange(persistedState.roll, -180, 180)
            : undefined,
      }
    : null;

  const initialAngles = {
    yaw: options.initialAngles?.yaw ?? persistedAngles?.yaw ?? 20,
    pitch: options.initialAngles?.pitch ?? persistedAngles?.pitch ?? 12,
    roll: options.initialAngles?.roll ?? persistedAngles?.roll ?? -18,
  };

  const initialConfidence = clampRange(
    options.initialConfidence ??
      (typeof persistedState?.confidence === 'number' && Number.isFinite(persistedState.confidence)
        ? persistedState.confidence
        : 0.85),
    0,
    1
  );
  ...
  const confidence = createSlider(controls, 'Confidence', { min: 0, max: 1, step: 0.05, value: initialConfidence });

  const shareCard = document.createElement('section');
  shareCard.className = 'qp-share-card';

  const shareHeading = document.createElement('h3');
  shareHeading.textContent = 'Share & restore';
  shareCard.appendChild(shareHeading);

  const shareDescription = document.createElement('p');
  shareDescription.className = 'qp-share-description';
  shareDescription.textContent =
    'Copy the current preview configuration as JSON or paste exported data to restore it on another machine.';
  shareCard.appendChild(shareDescription);

  const shareTextarea = document.createElement('textarea');
  shareTextarea.className = 'qp-share-textarea';
  shareTextarea.rows = 8;
  shareTextarea.wrap = 'off';
  shareTextarea.spellcheck = false;
  shareTextarea.placeholder = '{ "version": 1, … }';
  shareTextarea.setAttribute('aria-label', 'Quaternion preview state JSON');
  shareCard.appendChild(shareTextarea);

  const shareActions = document.createElement('div');
  shareActions.className = 'qp-share-actions';
  shareCard.appendChild(shareActions);

  const shareCopyButton = document.createElement('button');
  shareCopyButton.type = 'button';
  shareCopyButton.className = 'qp-button';
  shareCopyButton.textContent = 'Copy current JSON';
  shareActions.appendChild(shareCopyButton);

  const shareApplyButton = document.createElement('button');
  shareApplyButton.type = 'button';
  shareApplyButton.className = 'qp-button-secondary';
  shareApplyButton.textContent = 'Apply pasted JSON';
  shareActions.appendChild(shareApplyButton);

  const shareStatus = document.createElement('p');
  shareStatus.className = 'qp-share-status';
  shareCard.appendChild(shareStatus);

  controls.appendChild(shareCard);

  let shareStatusTimer: number | null = null;

  const setShareStatus = (message: string, tone: 'info' | 'success' | 'error') => {
    shareStatus.textContent = message;
    shareStatus.setAttribute('data-tone', tone);
    if (typeof window === 'undefined') {
      return;
    }
    ...
    shareStatusTimer = window.setTimeout(() => {
      ...
      shareStatus.removeAttribute('data-tone');
      shareStatusTimer = null;
    }, 3000);
  };

  shareCopyButton.addEventListener('click', async () => {
    const serialized = stringifyPreviewState(collectPreviewState(), true);
    shareTextarea.value = serialized;
    ...
    if (typeof navigator !== 'undefined' && navigator.clipboard?.writeText) {
      try {
        ...
        console.warn('[QuaternionPreview] Clipboard copy failed', error);
      }
    }
    ...
    if (!copied && typeof document !== 'undefined') {
      ...
      shareTextarea.setSelectionRange(0, 0);
      shareTextarea.blur();
    }

    if (copied) {
      setShareStatus('Copied preview JSON to clipboard.', 'success');
    } else {
      setShareStatus('Clipboard unavailable — JSON updated below.', 'info');
    }
  });

  shareApplyButton.addEventListener('click', () => {
    const parsed = parsePreviewState(shareTextarea.value.trim());
    if (!parsed) {
      setShareStatus('Unable to parse preview JSON payload.', 'error');
      return;
    }
    applyImportedState(parsed);
    setShareStatus('Restored preview configuration from JSON.', 'success');
  });
  ...
  if (storage) {
    ...
    const persistState = () => {
      storage.save(collectPreviewState());
    };
    schedulePersist = () => {
      updateShareTextarea();
      if (typeof window === 'undefined') {
  ...
  const systemsEntries = SYSTEM_BLUEPRINTS.map(({ id, label, defaults }) => {
    const system = new MockQuaternionSystem(label, systemsColumn, defaults);
    system.setActive(id === activeSystemId);
    return [id, system] as const;
  });
  ...
  const synchronizer = new ShaderQuaternionSynchronizer({
    ...
    targetSystems: [activeSystemId]
  });

  const applyActiveSystem = (systemId: SystemId) => {
    if (activeSystemId === systemId) {
      synchronizer.activateExclusiveSystem(systemId);
      return;
    }
    activeSystemId = systemId;
    synchronizer.activateExclusiveSystem(systemId);
    SYSTEM_BLUEPRINTS.forEach(({ id }) => {
      const system = systems[id];
      system.setActive(id === systemId);
      const radio = systemToggleInputs.get(id);
      if (radio) {
  ...
  SYSTEM_BLUEPRINTS.forEach(({ id }) => {
    const input = systemToggleInputs.get(id);
    input?.addEventListener('change', () => {
      if (input.checked) {
        applyActiveSystem(id);
      }
    });
  });

  applyActiveSystem(activeSystemId);
  ...
  function updateShareTextarea(force: boolean = false) {
    if (
      !force &&
      typeof document !== 'undefined' &&
      document.activeElement === shareTextarea
    ) {
      return;
    }
    shareTextarea.value = stringifyPreviewState(collectPreviewState(), true);
  }
  ...
  function applyImportedState(payload: PreviewStatePayload) {
    const presetCandidate = PREVIEW_PRESETS.find(preset => preset.id === payload.activePresetId);
    if (presetCandidate) {
      activePresetId = presetCandidate.id;
      presetSelect.select.value = presetCandidate.id;
      presetDescription.textContent = presetCandidate.description;
    } else {
      activePresetId = CUSTOM_PRESET_ID;
      presetSelect.select.value = CUSTOM_PRESET_ID;
      presetDescription.textContent = customPresetDescription;
    ...
    LAYER_BLUEPRINTS.forEach((blueprint, index) => {
      ...
      const layerPayload = payload.layers[index];
      const nextGeometry =
        layerPayload && typeof layerPayload.geometry === 'string' && isGeometryModule(layerPayload.geometry)
          ? layerPayload.geometry
          : blueprint.geometry;
      const nextProjection =
        layerPayload && typeof layerPayload.projection === 'string' && isProjectionModule(layerPayload.projection)
          ? layerPayload.projection
          : blueprint.projection;

      if (state.geometry !== nextGeometry || state.projection !== nextProjection) {
        geometryChanged = true;
      }
      ...
      const fallbackMaterial = createDefaultLayerMaterialControl(index, LAYER_BLUEPRINTS.length);
      const sanitizedMaterial = layerPayload
        ? sanitizeMaterialControl(layerPayload.material, fallbackMaterial)
        : fallbackMaterial;
      state.material = sanitizedMaterial;
    ...
    selectedGeometry =
      typeof payload.selectedGeometry === 'string' && isGeometryModule(payload.selectedGeometry)
        ? payload.selectedGeometry
        : layerStates[0]?.geometry ?? selectedGeometry;
    selectedProjection =
      typeof payload.selectedProjection === 'string' && isProjectionModule(payload.selectedProjection)
    ...
    setSliderHandleValue(yaw, clampRange(payload.yaw, -180, 180));
    setSliderHandleValue(pitch, clampRange(payload.pitch, -90, 90));
    setSliderHandleValue(roll, clampRange(payload.roll, -180, 180));
    setSliderHandleValue(confidence, clampRange(payload.confidence, 0, 1));

    const nextSystemId: SystemId =
      typeof payload.activeSystemId === 'string' && isSystemId(payload.activeSystemId)
