...
const DEFAULT_JERK_SMOOTHING = 0.4;
const DEFAULT_ANGULAR_JERK_SMOOTHING = 0.35;
const DEFAULT_PREDICTION_HORIZON_MS = 33;
const DEFAULT_PREDICTION_BLEND = 0.5;
const ANGULAR_EPSILON = 1e-6;
const POSE_FLOAT_COUNT = 80;
...
function rotationVectorToQuaternion(vector) {
  if (!vector) {
    return { x: 0, y: 0, z: 0, w: 1 };
  }

  const [vx = 0, vy = 0, vz = 0] = vector;
  ...
  const axisScale = 1 / angle;
  const halfAngle = angle / 2;
  const sinHalf = Math.sin(halfAngle);
  const cosHalf = Math.cos(halfAngle);

  if (!Number.isFinite(axisScale) || !Number.isFinite(sinHalf) || !Number.isFinite(cosHalf)) {
    return { x: 0, y: 0, z: 0, w: 1 };
  }

  return normalizeQuaternionObject({
    x: vx * axisScale * sinHalf,
    y: vy * axisScale * sinHalf,
    z: vz * axisScale * sinHalf,
    w: cosHalf,
  });
}

function predictOrientation(base, rotationVector, blend) {
  const delta = rotationVectorToQuaternion(rotationVector);
  const predicted = normalizeQuaternionObject(multiplyQuaternions(delta, base));
  const weight = clamp01(blend, 1);
  return weight === 1 ? predicted : slerpQuaternion(base, predicted, weight);
}
...
export class WebGPUGlassmorphicPipeline {
  constructor(device, options = {}) {
    ...
    this.jerkSmoothingFactor = clamp01(options.jerkSmoothingFactor ?? DEFAULT_JERK_SMOOTHING, DEFAULT_JERK_SMOOTHING);
    this.angularJerkSmoothingFactor = clamp01(options.angularJerkSmoothingFactor ?? DEFAULT_ANGULAR_JERK_SMOOTHING, DEFAULT_ANGULAR_JERK_SMOOTHING);
    this.predictionHorizonSeconds = Math.max(0, Number(options.predictionHorizonMs ?? DEFAULT_PREDICTION_HORIZON_MS) / 1000);
    this.predictionBlendFactor = clamp01(options.predictionBlendFactor ?? DEFAULT_PREDICTION_BLEND, DEFAULT_PREDICTION_BLEND);
    ...
    this.predictionRotationScratch = new Float32Array(3);
    ...
    this.previousSmoothedJerk = null;
    this.previousSmoothedAngularJerk = null;
    ...
    this.lastAngularAcceleration = null;
  }
  ...
  updatePose(pose = {}, metadata = {}) {
    ...
    const jerk = this.lastAcceleration && deltaSeconds > 0
      ? acceleration.map((value, index) => (value - this.lastAcceleration[index]) / deltaSeconds)
      : [0, 0, 0];

    const angularJerk = this.lastAngularAcceleration && deltaSeconds > 0
      ? angularAcceleration.map((value, index) => (value - this.lastAngularAcceleration[index]) / deltaSeconds)
      : [0, 0, 0];
    ...
    const smoothedJerk = mixVectors(this.previousSmoothedJerk, jerk, this.jerkSmoothingFactor);
    const smoothedAngularJerk = mixVectors(this.previousSmoothedAngularJerk, angularJerk, this.angularJerkSmoothingFactor);

    const predictedPosition = smoothedPosition.slice();
    const predictedVelocity = smoothedVelocity.slice();
    const predictedAngularVelocity = smoothedAngularVelocity.slice();
    const predictionRotation = this.predictionRotationScratch;
    predictionRotation[0] = 0;
    predictionRotation[1] = 0;
    predictionRotation[2] = 0;
    let predictedOrientation = smoothed;
    let predictedSpeed = Math.hypot(...predictedVelocity);
    let predictedAngularSpeed = Math.hypot(...predictedAngularVelocity);

    const horizonSeconds = this.predictionHorizonSeconds;
    if (horizonSeconds > 0) {
      const horizonSquared = horizonSeconds * horizonSeconds;
      const horizonCubed = horizonSquared * horizonSeconds;

      for (let index = 0; index < 3; index += 1) {
        predictedPosition[index] = smoothedPosition[index]
          + smoothedVelocity[index] * horizonSeconds
          + 0.5 * smoothedAcceleration[index] * horizonSquared
          + (1 / 6) * smoothedJerk[index] * horizonCubed;

        predictedVelocity[index] = smoothedVelocity[index]
          + smoothedAcceleration[index] * horizonSeconds
          + 0.5 * smoothedJerk[index] * horizonSquared;

        predictedAngularVelocity[index] = smoothedAngularVelocity[index]
          + smoothedAngularAcceleration[index] * horizonSeconds
          + 0.5 * smoothedAngularJerk[index] * horizonSquared;

        predictionRotation[index] = smoothedAngularVelocity[index] * horizonSeconds
          + 0.5 * smoothedAngularAcceleration[index] * horizonSquared
          + (1 / 6) * smoothedAngularJerk[index] * horizonCubed;
      }

      predictedOrientation = predictOrientation(smoothed, predictionRotation, this.predictionBlendFactor);
      predictedSpeed = Math.hypot(...predictedVelocity);
      predictedAngularSpeed = Math.hypot(...predictedAngularVelocity);
    }
    ...
    this.previousSmoothedJerk = smoothedJerk;
    this.previousSmoothedAngularJerk = smoothedAngularJerk;
    this.lastPosition = position.slice();
    this.lastVelocity = velocity.slice();
    this.lastOrientation = { ...orientation };
    this.lastAngularVelocity = angularVelocity.slice();
    this.lastAcceleration = acceleration.slice();
    this.lastAngularAcceleration = angularAcceleration.slice();
    ...
    poseArray[49] = jerk[1];
    poseArray[50] = jerk[2];
    poseArray[51] = Math.hypot(...jerk);

    poseArray[52] = smoothedJerk[0];
    poseArray[53] = smoothedJerk[1];
    poseArray[54] = smoothedJerk[2];
    poseArray[55] = this.jerkSmoothingFactor;

    poseArray[56] = angularJerk[0];
    poseArray[57] = angularJerk[1];
    poseArray[58] = angularJerk[2];
    poseArray[59] = Math.hypot(...angularJerk);

    poseArray[60] = smoothedAngularJerk[0];
    poseArray[61] = smoothedAngularJerk[1];
    poseArray[62] = smoothedAngularJerk[2];
    poseArray[63] = this.angularJerkSmoothingFactor;

    poseArray[64] = predictedPosition[0];
    poseArray[65] = predictedPosition[1];
    poseArray[66] = predictedPosition[2];
    poseArray[67] = horizonSeconds;

    poseArray[68] = predictedVelocity[0];
    poseArray[69] = predictedVelocity[1];
    poseArray[70] = predictedVelocity[2];
    poseArray[71] = predictedSpeed;

    poseArray[72] = predictedOrientation.x;
    poseArray[73] = predictedOrientation.y;
    poseArray[74] = predictedOrientation.z;
    poseArray[75] = predictedOrientation.w;

    poseArray[76] = predictedAngularVelocity[0];
    poseArray[77] = predictedAngularVelocity[1];
    poseArray[78] = predictedAngularVelocity[2];
    poseArray[79] = predictedAngularSpeed;
