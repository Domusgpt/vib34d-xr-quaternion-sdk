...
const DEFAULT_SNAP_SMOOTHING = 0.35;
const DEFAULT_ANGULAR_SNAP_SMOOTHING = 0.3;
...
export class WebGPUGlassmorphicPipeline {
  constructor(device, options = {}) {
    ...
    this.snapSmoothingFactor = clamp01(options.snapSmoothingFactor ?? DEFAULT_SNAP_SMOOTHING, DEFAULT_SNAP_SMOOTHING);
    this.angularSnapSmoothingFactor = clamp01(options.angularSnapSmoothingFactor ?? DEFAULT_ANGULAR_SNAP_SMOOTHING, DEFAULT_ANGULAR_SNAP_SMOOTHING);
    ...
    this.lastPoseSnapshot = null;
    this.lastMatrixSnapshot = null;
    this.lastAudioSnapshot = null;
    ...
    this.previousSmoothedSnap = null;
    this.previousSmoothedAngularSnap = null;
  ...
  updatePose(pose = {}, metadata = {}) {
    ...
    const snap = this.lastJerk && deltaSeconds > 0
      ? jerk.map((value, index) => (value - this.lastJerk[index]) / deltaSeconds)
      : [0, 0, 0];

    const angularSnap = this.lastAngularJerk && deltaSeconds > 0
      ? angularJerk.map((value, index) => (value - this.lastAngularJerk[index]) / deltaSeconds)
      : [0, 0, 0];
    ...
    const smoothedSnap = mixVectors(this.previousSmoothedSnap, snap, this.snapSmoothingFactor);
    const smoothedAngularSnap = mixVectors(this.previousSmoothedAngularSnap, angularSnap, this.angularSnapSmoothingFactor);
    ...
    this.previousSmoothedSnap = smoothedSnap;
    this.previousSmoothedAngularSnap = smoothedAngularSnap;
    ...
    this.lastJerk = jerk.slice();
    this.lastAngularJerk = angularJerk.slice();
    ...
    poseArray[65] = snap[1];
    poseArray[66] = snap[2];
    poseArray[67] = Math.hypot(...snap);

    poseArray[68] = smoothedSnap[0];
    poseArray[69] = smoothedSnap[1];
    poseArray[70] = smoothedSnap[2];
    poseArray[71] = this.snapSmoothingFactor;

    poseArray[72] = angularSnap[0];
    poseArray[73] = angularSnap[1];
    poseArray[74] = angularSnap[2];
    poseArray[75] = Math.hypot(...angularSnap);

    poseArray[76] = smoothedAngularSnap[0];
    poseArray[77] = smoothedAngularSnap[1];
    poseArray[78] = smoothedAngularSnap[2];
    poseArray[79] = this.angularSnapSmoothingFactor;

    this.poseUniforms.update(this.device, poseArray);
    this.lastPoseSnapshot = Float32Array.from(poseArray);
    ...
    this.lastMatrixSnapshot = Float32Array.from(matrixData);
  }
  ...
  updateAudio(frequencies) {
    ...
    this.lastAudioSnapshot = Float32Array.from(data);
  }
  ...
  getPoseSnapshot() {
    if (!this.lastPoseSnapshot) {
      return null;
    }
    return Float32Array.from(this.lastPoseSnapshot);
  }

  getMatrixSnapshot() {
    if (!this.lastMatrixSnapshot) {
      return null;
    }
    return Float32Array.from(this.lastMatrixSnapshot);
  }

  getAudioSnapshot() {
    if (!this.lastAudioSnapshot) {
      return null;
    }
    return Float32Array.from(this.lastAudioSnapshot);
  }

  getProductionSnapshot(metadata = {}) {
    return {
      metadata: { ...metadata },
      poseFloatCount: POSE_FLOAT_COUNT,
      poseUniforms: this.lastPoseSnapshot ? Array.from(this.lastPoseSnapshot) : null,
      matrixUniform: this.lastMatrixSnapshot ? Array.from(this.lastMatrixSnapshot) : null,
      audioUniform: this.lastAudioSnapshot ? Array.from(this.lastAudioSnapshot) : null,
    };
  }
}
