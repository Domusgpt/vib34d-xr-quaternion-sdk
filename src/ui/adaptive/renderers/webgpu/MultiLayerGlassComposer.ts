...
import type { GlassGeometryModule, GlassProjectionModule } from './GlassShaderLibrary.ts';

export interface LayerShaderConfig {
  /** Optional geometry module to drive the generated WGSL shader. */
  readonly geometry?: GlassGeometryModule;
  /** Optional projection module for mapping 4D points into 3D space. */
  readonly projection?: GlassProjectionModule;
  /** Provide explicit WGSL when bypassing the shader library helpers. */
  readonly code?: string;
}

export interface LayerMaterialConfig {
  /** Optional base colors for the layer's shader. */
  ...
  /** Pattern/glitch/color shift scalars used by dynamic WGSL shaders. */
  readonly patternIntensity?: number;
  readonly glitchIntensity?: number;
  readonly colorShift?: number;
  /** Lattice scalars (grid density, line thickness, shell width, tetra thickness). */
...
export interface LayerDescriptor {
  ...
  /** Optional material configuration for dynamic shader pipelines. */
  readonly material?: LayerMaterialConfig;
  /** Optional shader configuration describing geometry/projection modules. */
  readonly shader?: LayerShaderConfig;
}
