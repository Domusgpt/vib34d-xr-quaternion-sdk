...
export function composeQuaternionUniformBlock(dq: DualQuaternion): Float32Array {
  ...
  writeDualQuaternionToArray(dq, block, 0);
  return block;
}

export function writeDualQuaternionToArray(
  dq: DualQuaternion,
  target: Float32Array,
  offset = 0
): void {
  const real = normalize(dq.real);
  const dual = normalize(dq.dual);
  target[offset + 0] = real[0];
  target[offset + 1] = real[1];
  target[offset + 2] = real[2];
  target[offset + 3] = real[3];
  target[offset + 4] = dual[0];
  target[offset + 5] = dual[1];
  target[offset + 6] = dual[2];
  target[offset + 7] = dual[3];
}
