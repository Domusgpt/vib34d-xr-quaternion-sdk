  ...
  return {
    ...
    update(_device, data) {
      ...
      if (label === 'pose-uniforms') {
        ...
        const predictedPosition = snapshot.slice(64, 67);
        const predictedVelocity = snapshot.slice(68, 71);
        const predictedOrientation = snapshot.slice(72, 76);
        const predictedAngularVelocity = snapshot.slice(76, 79);
        ...
        console.log('  ↳ predicted position  :', predictedPosition, 'horizon=', snapshot[67]);
        console.log('  ↳ predicted velocity  :', predictedVelocity, 'speed=', snapshot[71]);
        console.log('  ↳ predicted orientation quaternion:', predictedOrientation);
        console.log('  ↳ predicted angular velocity:', predictedAngularVelocity, 'speed=', snapshot[79]);
      }
    },
  };
}
