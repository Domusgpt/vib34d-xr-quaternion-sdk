...
const ROTATION_PARAMS = ['rot4dXY', 'rot4dXZ', 'rot4dYZ', 'rot4dXW', 'rot4dYW', 'rot4dZW'];
...
export class ShaderQuaternionSynchronizer {
    ...
    applyToSystem(systemName, rotationTarget, context) {
        ...
        const queueUpdate = (param, targetValue) => {
            ...
            if (Object.prototype.hasOwnProperty.call(this.rotationState, param)) {
                this.rotationState[param] = computed.value;
            }
        };
        ...
        const rotationUniforms = this.buildRotationUniforms();
        const contextForSystem = {
            ...context,
            systemName,
            parameters: parameterMeta,
            ...(rotationUniforms ? { rotationUniforms } : {})
        };
    ...
    buildRotationUniforms() {
        if (!this.rotationState) {
            return null;
        }
        const hasAll = ROTATION_PARAMS.every(param => Number.isFinite(this.rotationState[param]));
        if (!hasAll) {
            return null;
        }
        try {
            return composeFourDRotation(this.rotationState);
        } catch (error) {
            this.logger?.warn?.('[ShaderQuaternionSynchronizer] failed to compose 4D rotation matrix', error);
            return null;
        }
    }
