...
// Each update context now carries rotationUniforms with matrix4/matrix3/planeAngles fields
quantum.batchUpdate = (updates, context) => {
    if (context.rotationUniforms) {
        useMatrix(context.rotationUniforms.matrix4);
    }
    applyQuantumParameters(updates);
};
```
