...
- **Exclusive activation** – Canvas transitions emit `vib34d:system-activated` / `vib34d:system-deactivated` events so only the currently visible engine receives quaternion deltas. The synchronizer listens for those events, keeps its target set in sync, and prevents simultaneous canvas allocation clashes. The preview harness mirrors this by dispatching the same events from its system selector so developers can swap engines without running multiple canvas stacks.【F:src/core/CanvasManager.js†L12-L74】【F:src/ui/adaptive/renderers/ShaderQuaternionSynchronizer.js†L20-L148】【F:src/dev/quaternionPreview.ts†L533-L629】
