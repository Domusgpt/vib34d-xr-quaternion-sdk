...
- **Exclusive activation** – Canvas transitions emit `vib34d:system-activated` / `vib34d:system-deactivated` events so only the currently visible engine receives quaternion deltas. The synchronizer listens for those events, keeps its target set in sync, and prevents simultaneous canvas allocation clashes. The preview harness mirrors the runtime by exposing a radio toggle for selecting the single active visualization system.【F:src/core/CanvasManager.js†L12-L74】【F:src/ui/adaptive/renderers/ShaderQuaternionSynchronizer.js†L20-L148】【F:src/dev/quaternionPreview.ts†L742-L876】
