...
- **Exclusive activation** – Canvas transitions emit `vib34d:system-activated` / `vib34d:system-deactivated` events so only the currently visible engine receives quaternion deltas. The synchronizer listens for those events, keeps its target set in sync, and prevents simultaneous canvas allocation clashes. To protect builds that have not yet wired the lifecycle events, the synchronizer now caps the active target list to a single system by default (override with `maxActiveSystems`). The preview harness mirrors the runtime by exposing a radio toggle for selecting the single active visualization system. The `CanvasManager` also listens for viewport/orientation changes so it can rescale the active canvas stack and notify the engine of the new pixel budget, keeping holographic fidelity stable during window resizes or headset rotations.【F:src/core/CanvasManager.js†L8-L209】【F:src/ui/adaptive/renderers/ShaderQuaternionSynchronizer.js†L20-L214】【F:src/dev/quaternionPreview.ts†L742-L876】
