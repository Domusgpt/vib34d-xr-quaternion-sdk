...
### 1.4 Testing Activation
- After `npm install`, run `npm test` to execute the Node test suite (`node --test`). The harness now includes geometry,
  parameter, quaternion, and Vib3+ environment coverage.
- Use `npm run setup` first if you need Node version validation and dependency hydration in one step.
- Additional integration tests will land during Phase 6; record device-specific test plans once profiling targets are defined.
