...
- `SpatialStoryGraph` listens to localization fabric events, rotor fusion metrics, and audio spikes, then emits plug-in triggers that adjust audio/visual uniforms or pulse boosts before they hit the composer. Its state feeds the preview harness overlays so designers see which emergent behaviors are active in real time.【F:src/ui/adaptive/localization/SpatialStoryGraph.ts†L1-L247】【F:src/dev/webgpuPreviewHarness.ts†L1-L420】
- `PredictiveRotorCache` runs a Kalman-filtered orientation model that forecasts XW/YW/ZW rotor values a frame ahead, blending them into the controller so late-latching budgets stay on track while exposing horizon/confidence telemetry to the preview UI.【F:src/ui/adaptive/localization/PredictiveRotorCache.ts†L1-L204】【F:src/ui/adaptive/renderers/webgpu/GlassUniformController.ts†L1-L331】
...
- The preview sandbox exposes geometry/projection selectors that rebuild either the WebGPU pipelines or the WebGL fallback so shader authors can compare lattice mixes across render paths without restarting the harness.【F:src/dev/quaternionPreview.ts†L112-L160】【F:src/dev/quaternionPreview.ts†L312-L400】
...
- `LayerBlurHelper` and its shared `SeparableBlurPipeline` allocate half-resolution ping-pong textures, write Gaussian kernel uniforms, and issue horizontal/vertical passes so preview and XR runtimes share identical blur outputs prior to compositing.【F:src/ui/adaptive/renderers/webgpu/LayerBlurHelper.ts†L1-L187】【F:src/ui/adaptive/renderers/webgpu/SeparableBlurPipeline.ts†L1-L127】
