...
- `GlassShaderLibrary` generates WGSL layer shaders for the hypercube/hypersphere/hypertetra lattice family, mirroring the legacy WebGL math so geometry + projection choices stay in sync across preview and immersive pipelines.【F:src/ui/adaptive/renderers/webgpu/GlassShaderLibrary.ts†L1-L366】
