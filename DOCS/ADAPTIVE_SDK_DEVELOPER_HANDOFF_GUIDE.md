...
3. **Telemetry Providers & Middleware**
   ...
   - Configure batching via `telemetry.batch` (max size, age, buffer cap) so XR runtimes can coalesce high-volume events before dispatch. Use `sdk.telemetryControls.flushBatches()` during shutdown sequences and prefer `deliverBatch` for providers that expose batched ingestion hooks.【F:src/product/ProductTelemetryHarness.js†L1-L200】【F:src/product/telemetry/TelemetryBatcher.js†L1-L200】【F:tests/productTelemetryHarness.batch.test.ts†L1-L120】
