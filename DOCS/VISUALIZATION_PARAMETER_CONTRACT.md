...
## Rotation Management Helpers

`ParameterManager` exposes utility methods so producers can manipulate the full
six-plane rotation state without manually iterating plane names:

- `setRotationPlane(plane, value)` – clamps and stores a single plane, throwing
  if an unknown plane key is provided.
- `getRotationParameters()` – returns a fresh object containing all six plane
  values, suitable for snapshots or downstream serialization.
- `applyRotationProfile(profile)` – applies any subset of plane values from a
  profile object or `{ rotations: { ... } }` wrapper, invalidating the cached
  rotation snapshot automatically.
- `applyRotationDeltas(deltas)` – increments the current rotation state by the
  provided delta values, reusing the same clamping and caching logic.【F:src/core/Parameters.js†L8-L210】

`Vib3PlusEnvironment` wraps these helpers so higher-level orchestration can
broadcast rotation changes in a single call:

- `getRotationState()` mirrors `ParameterManager.getRotationParameters()` for
  convenience.
- `setRotationPlane(plane, value, options)` optionally broadcasts a targeted
  update with the latest rotation snapshot in the context payload.
- `applyRotationProfile(profile, options)` writes the profile to the parameter
  store and sends every plane value to registered systems with `reason`,
  `rotationProfile`, and `rotationState` metadata for synchronized GPU updates.
- `applyRotationDeltas(deltas, options)` performs additive adjustments and
  broadcasts the refreshed state, including the delta payload for consumers that
  need to integrate with animation timelines.【F:src/vib3plus/Vib3PlusEnvironment.js†L1-L233】

These helpers make it trivial to seed presets from the Vib3+ geometry tree,
apply gesture-derived deltas, or stream XR quaternion decompositions directly
into the visualization systems without duplicating plane bookkeeping.
