...
test('ParameterManager keeps geometry base and core in sync', () => {
    ...
    assert.equal(manager.getParameter('geometry'), 3);

    manager.setParameters({ geometryBase: 2, geometryCore: 1 });
    assert.equal(manager.getParameter('geometryBase'), 2);
    assert.equal(manager.getParameter('geometryCore'), 1);
    assert.equal(
        manager.getParameter('geometry'),
        (1 * GeometryLibrary.baseGeometries.length) + 2
    );
