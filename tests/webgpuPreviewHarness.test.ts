...
describe('WebXRQuaternionBridge with mock frame', () => {
  it('packs pose, audio, and timing data into the composer uniform ring', () => {
    ...
    const palette = readField(WebXRQuaternionBridge.uniformLayout, uniforms!, 'materialPrimary');
    expect(palette[0]).toBeCloseTo(1, 4);
    expect(palette[1]).toBeCloseTo(0.2, 4);
    expect(palette[2]).toBeCloseTo(0.8, 4);
  });
});
