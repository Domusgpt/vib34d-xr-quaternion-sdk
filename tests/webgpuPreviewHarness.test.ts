...
describe('WebXRQuaternionBridge with mock frame', () => {
  it('packs pose, audio, and timing data into the composer uniform ring', () => {
    ...
    const composer = new MultiLayerGlassComposer({
      ...
      uniformSize: Float32Array.BYTES_PER_ELEMENT * 64
    });
    ...
    expect(uniforms?.[56]).toBeCloseTo(1.234, 4);
    expect(uniforms?.[57]).toBeCloseTo(0.016, 4);
    expect(uniforms?.[58]).toBeCloseTo(0.85, 4);
    expect(uniforms?.[60]).toBeCloseTo(0.25, 4);
    expect(uniforms?.[62]).toBeCloseTo(0.75, 4);
  });
});
