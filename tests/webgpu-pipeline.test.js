...
test('predicts pose horizon entries using smoothed motion metrics', () => {
  const poseRing = createRecordingRing('pose');
  const matrixRing = createRecordingRing('matrix');
  const audioRing = createRecordingRing('audio');
  ...
  const pipeline = new WebGPUGlassmorphicPipeline(device, {
    ...
    velocitySmoothingFactor: 1,
    accelerationSmoothingFactor: 1,
    angularVelocitySmoothingFactor: 1,
    angularAccelerationSmoothingFactor: 1,
    jerkSmoothingFactor: 1,
    angularJerkSmoothingFactor: 1,
    predictionHorizonMs: 1000,
    predictionBlendFactor: 1,
    uniformRingFactory: (_size, label) => {
      if (label === 'pose-uniforms') return poseRing;
      if (label === 'matrix-uniforms') return matrixRing;
      return audioRing;
  ...
  const predictedPosition = poseData.slice(64, 67);
  assert.ok(Math.abs(predictedPosition[0] - (16 / 3)) < 1e-6);
  assert.ok(Math.abs(predictedPosition[1]) < 1e-6);
  assert.ok(Math.abs(predictedPosition[2]) < 1e-6);
  assert.ok(Math.abs(poseData[67] - 1) < 1e-6);

  const predictedVelocity = poseData.slice(68, 71);
  assert.ok(Math.abs(predictedVelocity[0] - 5) < 1e-6);
  assert.ok(Math.abs(predictedVelocity[1]) < 1e-6);
  assert.ok(Math.abs(predictedVelocity[2]) < 1e-6);
  assert.ok(Math.abs(poseData[71] - 5) < 1e-6);

  const predictedOrientation = poseData.slice(72, 76);
  assert.ok(Math.abs(predictedOrientation[0]) < 1e-6);
  assert.ok(Math.abs(predictedOrientation[1] - Math.sin((2 * Math.PI) / 3)) < 1e-6);
  assert.ok(Math.abs(predictedOrientation[2]) < 1e-6);
  assert.ok(Math.abs(predictedOrientation[3] - Math.cos((2 * Math.PI) / 3)) < 1e-6);

  const predictedAngularVelocity = poseData.slice(76, 79);
  const expectedAngularSpeed = (5 * Math.PI) / 4;
  assert.ok(Math.abs(predictedAngularVelocity[1] - expectedAngularSpeed) < 1e-6);
  assert.ok(Math.abs(poseData[79] - expectedAngularSpeed) < 1e-6);
});
