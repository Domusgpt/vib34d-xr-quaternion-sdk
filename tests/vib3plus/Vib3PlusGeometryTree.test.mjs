...
test('Vib3Plus geometry helpers resolve by base/core identifiers', () => {
    const tree = buildVib3PlusGeometryTree([0, 1]);
    const sample = tree.find(node => node.base.key === 'torus' && node.core.key === 'hypersphere-core');
    assert.ok(sample);

    const described = describeVib3PlusGeometry({ baseKey: 'torus', coreKey: 'hypersphere-core' });
    assert.deepEqual(described, buildVib3PlusGeometryTree([0]).find(node => node.geometryIndex === sample.geometryIndex));

    const resolved = findVib3PlusGeometry({ baseKey: 'torus', coreKey: 'hypersphere-core' }, [0, 1]);
...
test('Vib3PlusEnvironment applies geometry nodes and broadcasts updates', () => {
    ...
    assert.ok(lastUpdate.context.rotationProfile);
    assert.equal(lastUpdate.context.rotationProfile.level, result.variation.level);
});
...
test('Vib3PlusEnvironment resolves geometry by components and exposes rotation profile', () => {
    const env = createVib3PlusEnvironment({ geometryLevels: [0, 1, 2] });
    const { node } = env.applyGeometryComponents({ baseKey: 'sphere', coreKey: 'hypertetra-core' }, { level: 2 });
    assert.equal(node.base.key, 'sphere');
    assert.equal(node.core.key, 'hypertetra-core');

    const profile = env.getRotationProfile({ level: 2 });
    assert.ok(profile);
    assert.equal(profile.level, 2);
    profile.planes.forEach(plane => {
        const paramName = plane.plane;
        assert.equal(
            env.parameterManager.getParameter(paramName),
            node.variations.find(entry => entry.level === 2).parameters[paramName]
        );
    });
});
