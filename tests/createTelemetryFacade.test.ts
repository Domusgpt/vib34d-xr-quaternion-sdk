...
describe('createTelemetryFacade', () => {
    ...
    it('exposes batching helpers directly from the harness', async () => {
        const flushBatches = vi.fn().mockResolvedValue({ batches: 1 });
        const deliverBatch = vi.fn();
        const harness = {
            flushBatches,
            deliverBatch,
            getTelemetryBatcher: vi.fn().mockReturnValue({ size: 0 })
        };

        const facade = createTelemetryFacade({ harness });
        await facade.flushBatches({ reason: 'test' });
        expect(flushBatches).toHaveBeenCalledWith({ reason: 'test' });

        const records = [{ event: 'test.event', classification: 'analytics' }];
        await facade.deliverBatch(records, { reason: 'manual' });
        expect(deliverBatch).toHaveBeenCalledWith(records, { reason: 'manual' });

        expect(facade.getTelemetryBatcher()).toMatchObject({ size: 0 });
    });
});
