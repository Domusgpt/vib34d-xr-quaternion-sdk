...
| 64-66 | Instantaneous translation snap vector (m/s⁴) |
| 67 | Translation snap magnitude |
| 68-70 | Smoothed translation snap |
| 71 | Translation snap smoothing factor |
| 72-74 | Instantaneous angular snap vector (rad/s⁴) |
| 75 | Angular snap magnitude |
| 76-78 | Smoothed angular snap |
| 79 | Angular snap smoothing factor |
...
Need turnkey reference outputs while iterating on shaders or XR integration layers? Generate reproducible pipeline and frame-loop captures that mirror the demo scenarios without manual copy/paste:

```bash
npm run demo:webgpu:examples
```

The command emits structured JSON snapshots under `DOCS/examples/`:

- `webgpu-pipeline-production.json` – Captures uniform deltas, render-pass order, and the mock texture setup for two representative frames of the glassmorphic renderer.
- `webgpu-xr-loop-production.json` – Records XR session events, audio/pose uniform streams, and the queued command buffers produced by the `WebGPUXRFrameLoop`.

Review `DOCS/examples/WEBGPU_PRODUCTION_SNAPSHOTS.md` for field-by-field descriptions and highlighted metrics so you can compare your live output against the canned samples.

### Interactive snapshot viewer

Prefer an at-a-glance, glassmorphic UI that mirrors the production pipeline? Spin up the static gallery and open it in your browser:

```bash
npm run demo:webgpu:examples   # refresh the JSON captures
npm run demo:webgpu:serve      # serve DOCS/examples at http://localhost:4173
```

Then visit [http://localhost:4173](http://localhost:4173) to explore the mock frame metrics, bundle summaries, and pose uniform highlights rendered with the same aesthetic used in the XR renderer.
