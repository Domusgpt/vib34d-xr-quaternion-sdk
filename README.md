...
Generate reusable reference productions that capture the pose, matrix, and audio uniforms written by the glassmorphic pipeline each frame. The exporter records three consecutive frames, rounds the resulting uniform data for readability, and writes the sample payload to `DOCS/productions/webgpu-glassmorphic-sample.json`:

```bash
npm run demo:webgpu:productions
```

Every run refreshes the JSON snapshot so you can diff the evolving uniform layout or share example productions with downstream teams. The output includes the per-pass render summary to show how layer, blur, and composite passes were executed for each frame.
