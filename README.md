...
- Adds per-layer material controls for colors, pattern/glitch intensity, and lattice densities so the preview tweaks the same uniform data the WGSL pipelines and WebGL fallback consume.【F:src/dev/quaternionPreview.ts†L112-L400】【F:src/dev/webgpuPreviewHarness.ts†L198-L360】【F:src/dev/webglFallbackPreview.ts†L1-L360】
- Introduces snapshot presets so designers can copy, share, and import preview configurations with full layer fidelity through JSON state exports.【F:src/dev/quaternionPreview.ts†L344-L626】【F:src/dev/previewState.ts†L1-L176】
